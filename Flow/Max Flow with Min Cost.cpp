...
//iniatialize: 	mcmf.init(total_node_need+1);
//Edge:		   	mcmf.AddEdge(u,v,cap,cost);
//ans:			mcmf.Mincost(source, sink);	//{cost,flow};

/*
	pair<T1,T2> Mincost2(int s,int t) {//bellmanFord
        T1 tot_cost = 0;
        T2 tot_flow = 0;
        while(true) {
            for(int i = 0; i < n; i++) d[i] = INF;
